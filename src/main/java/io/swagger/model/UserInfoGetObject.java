package io.swagger.model;

import java.util.Objects;
import com.fasterxml.jackson.annotation.JsonProperty;
import com.fasterxml.jackson.annotation.JsonCreator;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import io.swagger.model.AdditionalClaim;
import io.swagger.model.UserInfoPutObject;
import java.util.List;
import org.springframework.validation.annotation.Validated;
import javax.validation.Valid;
import javax.validation.constraints.*;

/**
 * UserInfoGetObject
 */
@Validated
@javax.annotation.Generated(value = "io.swagger.codegen.v3.generators.java.SpringCodegen", date = "2020-07-03T15:24:00.359Z[GMT]")
public class UserInfoGetObject extends UserInfoPutObject  {
  @JsonProperty("fiscal_number")
  private String fiscalNumber = null;

  public UserInfoGetObject fiscalNumber(String fiscalNumber) {
    this.fiscalNumber = fiscalNumber;
    return this;
  }

  /**
   * Codice fiscale
   * @return fiscalNumber
  **/
  @ApiModelProperty(required = true, value = "Codice fiscale")
      @NotNull

  @Size(min=16,max=16)   public String getFiscalNumber() {
    return fiscalNumber;
  }

  public void setFiscalNumber(String fiscalNumber) {
    this.fiscalNumber = fiscalNumber;
  }


  @Override
  public boolean equals(java.lang.Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    UserInfoGetObject userInfoGetObject = (UserInfoGetObject) o;
    return Objects.equals(this.fiscalNumber, userInfoGetObject.fiscalNumber) &&
        super.equals(o);
  }

  @Override
  public int hashCode() {
    return Objects.hash(fiscalNumber, super.hashCode());
  }

  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class UserInfoGetObject {\n");
    sb.append("    ").append(toIndentedString(super.toString())).append("\n");
    sb.append("    fiscalNumber: ").append(toIndentedString(fiscalNumber)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }
}
